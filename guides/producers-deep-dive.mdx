...
description: "A deep-dive into Kafka producer internals: message batching, compression algorithms, idempotent producers, transactions, and exactly-once semantics with code examples."
---
...
Your Application Thread
        ...
        ▼
┌───────────────────┐
│  KafkaProducer    │  serialize key/value, determine partition
└────────┬──────────┘
         │
         ▼
┌───────────────────┐
│ RecordAccumulator │  buffer records into per-partition ProducerBatches
│  [batch P0]       │
│  [batch P1]       │
│  [batch P2]       │
└────────┬──────────┘
         │  drain when batch.size reached OR linger.ms expires
         ▼
┌───────────────────┐
│   Sender Thread   │  background I/O loop
└────────┬──────────┘
         │
         ▼
┌───────────────────┐
│   NetworkClient   │  manages TCP connections to brokers
└────────┬──────────┘
         │
         ▼
      Kafka Broker
...
Producer                              Broker
   │                                     │
   │── ProduceRequest (PID=42, seq=1) ──▶│  Write record, remember seq=1
   │◀─ Ack ──────────────────────────────│
   │                                     │
   │── ProduceRequest (PID=42, seq=2) ──▶│  Network timeout; producer retries
   │── ProduceRequest (PID=42, seq=2) ──▶│  Duplicate! seq=2 already seen → DISCARD
   │◀─ Ack ──────────────────────────────│
```
